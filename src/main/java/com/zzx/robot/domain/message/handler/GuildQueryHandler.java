...
public class GuildQueryHandler implements MessageHandler {
    ...
    public boolean match(String type, String message) {
        return "guild".equals(type) && message.startsWith("查询");
    }
    ...
    public void handle(WebSocketSession session, JsonNode message) {
        try {
            ...
            ObjectNode guildMessage = MessageConstructor.newGuildMessage(message.get("guild_id").asText(), message.get("channel_id").asText(), robotMessage);
            messageSender.send(session, new TextMessage(guildMessage.toString()));
