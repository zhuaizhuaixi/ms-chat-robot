...
public class GroupQueryHandler implements MessageHandler {
    ...
    public boolean match(String type, String message) {
        return "group".equals(type) && message.startsWith("查询");
    }
    ...
    public void handle(WebSocketSession session, JsonNode message) {
        try {
            ...
            if ("我".equals(characterName)) {
                String qq = message.get("user_id").asText();
                User user = userRepository.findByQq(qq);
                if (user == null || !StringUtils.hasText(user.getMapleName())) {
                    ObjectNode groupMessage = MessageConstructor.newGroupMessage(groupId, "未绑定角色");
                    messageSender.send(session, new TextMessage(groupMessage.toString()));
                    return;
                } else {
                    characterName = user.getMapleName();
                }
            }
            ...
            JsonNode coinNode = queryResult.get("CharacterData").get("LegionCoinsPerDay");
            if (coinNode != null) {
                legionCoins = queryResult.get("CharacterData").get("LegionCoinsPerDay").asText();
            } else {
                legionCoins = "小号，查不到";
            }
            ...
            ObjectNode groupMessage = MessageConstructor.newGroupMessage(groupId, robotMessage);
            messageSender.send(session, new TextMessage(groupMessage.toString()));

        } catch (Exception e) {
            logger.error("maple查询失败：", e);
        }
